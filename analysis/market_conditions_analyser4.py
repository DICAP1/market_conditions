...
import os
import json
# Import redis_client from your app/__init__.py
from app import redis_client # This import assumes redis_client is initialized in app/__init__.py

# Define a prefix for keys storing the last emitted payload to avoid conflicts
LAST_EMITTED_KEY_PREFIX = "last_emitted_analysis:"
# Define the TTL (Time To Live) for Redis keys in seconds
REDIS_KEY_TTL_SECONDS = 90
...
def fetch_historical_data(instrument, granularity):
    ...
    Args:
        ...
        granularity (str): Timeframe granularity (e.g., 'M15', 'H1', 'D')
...
def analyze_instruments(socketio=None):
    """
    Analyze all configured instruments and store results in Redis.
    
    This function performs comprehensive market analysis on all instruments,
    stores results in Redis for API access, and optionally emits real-time
    updates via SocketIO. It includes feature selection, price prediction,
    ...
    """
    analysis_data = {}
    all_signals = []  # Collect signals for final strategy
    ...
    for inst in OANDA_INSTRUMENTS:
        ...
        df = fetch_historical_data(inst, "M15")
        if df.empty:
            continue
        ...
        # Append current instrument signals for overall strategy
        all_signals.append([trend, momentum, macd, breakout, volatility, spread])

        result = {
            "predicted_price": pred,
            "best_features": best_feat,
        ...
        # Compute and add final strategy (from all signals)
        final_strat = final_strategy_decision(all_signals)
        result["final_strategy"] = final_strat

        # Store result in Redis
        redis_key = f"analysis:{inst}"
        try:
            redis_client.set(redis_key, json.dumps(result), ex=REDIS_KEY_TTL_SECONDS)
        except Exception as e:
            print(f"Error setting Redis key {redis_key}: {e}")
        ...
        last_emitted_key = f"{LAST_EMITTED_KEY_PREFIX}{inst}"
        last_emitted_json = redis_client.get(last_emitted_key)
        last_emitted_payload = json.loads(last_emitted_json) if last_emitted_json else None

        if socketio and current_payload != last_emitted_payload:
            try:
                socketio.emit("analysis_update", current_payload, namespace='/')
                redis_client.set(last_emitted_key, json.dumps(current_payload), ex=REDIS_KEY_TTL_SECONDS)
                print(f"[WS EMIT] {inst}: {current_payload}")
            except Exception as e:
                print(f"Error emitting for {inst}: {e}")
        elif socketio:
            print(f"[WS SKIP EMIT] {inst}: identical payload")

        analysis_data[inst] = result

    all_signals.clear()  # Optional: reset after complete run
